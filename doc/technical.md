...
折叠地图不创建第二套 Three.js 场景。`src/main.js` 用 `Trail.nearest()` 把当前章节 3 个真实证据锚点换算成路线进度，再投影到同一条 SVG 测绘路径；5 个地标使用 HUD 相同的 `0.02 / 0.27 / 0.74 / 0.86 / 0.955` 进度阈值和章节名称。玩家针将相机 yaw 与当前位置的世界路径切线、SVG 路径切线组合，表达相对路线朝向；下一地标距离由真实 `Trail.length × 进度差` 估算。林冠、溪流、遗迹平面、水池、网格、等高线、北针与比例尺提供静态空间参照，玩家针、已走路线、线索状态和四章解锁状态来自实时游戏/存档。地图 DOM 只在打开时更新，关闭后没有逐帧开销。
