...
`Ambience.playClueHint()` 使用等功率 `PannerNode` 从证据世界位置播放短促双音，第三章因此天然支持左右方向比较；静音时 UI 外缘方位刻度提供等价视觉信息。地图纸面由 CSS 生成两条等权竖向折痕和一条浅横折痕，没有书脊或分页 DOM。所有 HUD、面板和章节按钮在 320×568 保持无溢出，触控动作至少 44×44 px。
